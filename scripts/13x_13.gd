...
func _generate_new_puzzle():
	...
	print("Generating new 13x13 puzzle...")
	...
	_update_loading_progress(1.0)  # 100% - complete
	
	await get_tree().create_timer(0.3).timeout  # Show completion briefly
...
func load_new_puzzle():
	...
	# Generate a NEW puzzle with loading screen
	_generate_new_puzzle()
...
func restart_current_puzzle():
	...
	if not current_puzzle_grid.is_empty():
		...
		# Load the stored puzzle
		var puzzle_data = puzzle_maker.load_generated_puzzle(current_puzzle_grid, self, grid_offset)
		_update_loading_progress(0.6)  # 60% - puzzle loaded
