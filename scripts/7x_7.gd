...
func _draw_bridges():
	for br in bridges:
		...
		if br.count == 2:
			...
			if br.start_pos.x == br.end_pos.x: # vertical
				draw_line(br.start_pos + Vector2(-3,0), br.end_pos + Vector2(-3,0), color, width)
				draw_line(br.start_pos + Vector2(3,0), br.end_pos + Vector2(3,0), color, width)
			else: # horizontal
				draw_line(br.start_pos + Vector2(0,-3), br.end_pos + Vector2(0,-3), color, width)
				draw_line(br.start_pos + Vector2(0,3), br.end_pos + Vector2(0,3), color, width)
		else:
			draw_line(br.start_pos, br.end_pos, color, width)
